...
function SettingsView({
  ...
  const sectionKeys = ["seat", "plans", "roles", "team", "categories", "profile", "notifications", "automation", "upi"];
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <section className="rounded-3xl border border-slate-100 dark:border-gray-800 bg-white dark:bg-gray-900 p-6 shadow-sm">
          <button
            ...
            className="flex w-full items-center justify-between rounded-2xl border border-transparent p-2 text-left transition hover:border-indigo-100"
            aria-expanded={openSections.upi}
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">UPI / QR Payment</h2>
                <p className="text-sm text-slate-500 dark:text-slate-400">
                  Configure the QR code shown to students for offline UPI payments.
                </p>
              </div>
            </div>
            <LucideIcon
              name={openSections.upi ? "ChevronUp" : "ChevronDown"}
              className="h-4 w-4 text-slate-500 dark:text-slate-400"
            />
          </button>
          {openSections.upi ? (
            <div className="mt-6 space-y-4">
              <div>
                ...
                <p className="mb-2 text-xs text-slate-500 dark:text-slate-400">
                  Paste a hosted image URL (e.g., your QR code PNG) or a UPI deep-link
                  (e.g., <code className="rounded bg-slate-100 px-1 dark:bg-gray-800">upi://pay?pa=you@upi&amp;pn=YourName</code>).
                  Students will see this QR when requesting an offline payment.
                </p>
                <input
                  type="text"
                  name="upiQrUrl"
                  ...
                  onChange={handleChange}
                  placeholder="https://your-host.com/qr.png  or  upi://pay?pa=..."
                  className="w-full rounded-xl border border-slate-200 dark:border-gray-700 bg-slate-50 dark:bg-gray-800 px-3 py-2 text-sm text-slate-700 dark:text-slate-200 outline-none transition focus:border-indigo-400 focus:bg-white dark:focus:bg-gray-700 focus:ring-2 focus:ring-indigo-100"
                />
              </div>
              {profile.upiQrUrl && profile.upiQrUrl.startsWith("http") ? (
                <div className="mt-3">
                  <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">QR Preview</p>
                  <img
                    src={profile.upiQrUrl}
                    alt="UPI QR Code Preview"
                    className="h-40 w-40 rounded-2xl border border-slate-200 dark:border-gray-700 object-contain p-1 shadow"
                    onError={(e) => { e.currentTarget.style.display = "none"; }}
                  />
                </div>
              ) : profile.upiQrUrl && profile.upiQrUrl.startsWith("upi://") ? (
                <div className="mt-3 rounded-2xl border border-emerald-100 bg-emerald-50 dark:bg-emerald-900/20 dark:border-emerald-800 px-4 py-3">
                  <p className="text-xs font-semibold text-emerald-700 dark:text-emerald-300">
                    UPI deep-link configured. Students will be prompted to open their UPI app.
                  </p>
                  <p className="mt-1 break-all text-xs text-emerald-600 dark:text-emerald-400">{profile.upiQrUrl}</p>
                </div>
              ) : null}
            </div>
          ) : null}
