...
function CoordinatorHomeView({
  ...
  const metrics = useMemo(() => {
    ...
    const dueThisWeek = students.filter((student) => {
      ...
      const dueDate = new Date(student.renewal_date);
      const now = new Date();
  ...
  }, [payments, pendingPayments.length, scheduledRequests.length, students]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Send payment requests, monitor renewals, and verify student
            payments.
          </p>
        </div>
        <button
      ...
      <div className="overflow-x-auto pb-1">
        <div className="grid min-w-[960px] grid-cols-[1.1fr_0.9fr] gap-4">
          <section className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
            ...
            <div className="mt-5 grid grid-cols-2 gap-3">
              {accessCards.map(([title, description]) => (
                <div
                  key={title}
